...
const ManageEnrollment = () => {
	...
	const handleEnrollStudent = async (event) => {
		...
		try {
			...
			// Handle enrollment error (show error message, etc.)
			console.error(error);
			console.log(error.response.data.message);
		}
	};

	const handleStudentClick = async (courseId, studentId) => {
		const confirmDelete = window.confirm(
		...
		try {
			const response = await axios.delete(
				`http://localhost:3001/api/remove-student/${courseId}/${studentId}`
			);
				...
				<div className="popup">
					<div className="popup-inner">
						...
						<form className="add-course-form" onSubmit={handleEnrollStudent}>
							{/* Add relevant fields for enrolling a student */}
